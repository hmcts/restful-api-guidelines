...
NOTE: The following is subject to change pending decisions on API
...
The first two ways are considered preferable, the third should only be
...
All Media Types defined must include a version to allow for future version
changes.
...
NOTE: This section is subject to change pending a decision on API
documentation standards.
